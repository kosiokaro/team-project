...
import interface_adapter.favorites.LoadFavoritesController;
import interface_adapter.favorites.LoadFavoritesState;
import interface_adapter.favorites.LoadFavoritesViewModel;
...
public class TestFavoritesView extends JPanel implements PropertyChangeListener {
    ...
    private LoadFavoritesViewModel loadViewModel;
    private FavoritesController controller;
    private LoadFavoritesController loadController;
    private String currentUsername;

    public final String viewName = "TESTFAVORITES";
    ...
    private Consumer<Integer> detailsClickListener;

    public TestFavoritesView(LoadFavoritesViewModel loadViewModel) {
        ...
        switchButton = new JButton("Go to Watchlist");
    ...
    private void attemptLoad() {
        if (currentUsername != null && loadController != null) {
            System.out.println("Attempting to load Favorites for: " + currentUsername);
            loadUserFavorites();
        } else {
    ...
    public void addNotify() {
        super.addNotify();
        System.out.println("FavoritesView.addNotify() called");
        attemptLoad();
    }

    public void loadUserFavorites() {
        System.out.println("=== loadUserFavorites called ===");
        ...
        if (loadController != null && currentUsername != null) {
            System.out.println("✓ Both conditions met, calling loadController.loadFavorites");
            loadController.loadFavorites(currentUsername);
        } else {
    ...
    public void setLoadController(LoadFavoritesController controller) {
    ...
    public void loadFavorites(List<Movie> movies) {
        System.out.println("=== loadFavorites called with " + (movies != null ? movies.size() : "NULL") + " movies ===");
        clearAllMedia();
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        System.out.println("FavoritesView propertyChange event: " + evt.getPropertyName());
        if ("state".equals(evt.getPropertyName())) {
            LoadFavoritesState state = (LoadFavoritesState) evt.getNewValue();
            if (state.getMovies() != null) {
                loadFavorites(state.getMovies());
            }
    ...
    private JPanel createMediaRow(String title, int id, List<String> genres,
        ...
        removeButton.addActionListener(e -> {
            if (controller != null && currentUsername != null) {
                controller.removeFromFavorites(currentUsername, String.valueOf(id));
    ...
    public void setDetailsClickListener(Consumer<Integer> listener) {
